...
`Balefire\Components\BrandIconCards\BrandIconCards` (PSR-4) and
`Balefire\Components\BrandIconCards\CardMedia` (private internal helper
...
`assets/brand-icon-cards.css` — uses `data-cols` attribute selector +
...
```css
@import "../../vendor/balefire/bma-brand-icon-cards/assets/brand-icon-cards.css";
```
